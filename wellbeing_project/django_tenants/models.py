...
class DomainMixin(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        # os.system('cp /etc/hosts {}/etc/hosts'.format(settings.STATICFILES_DIRS[0]))
        # with open(settings.STATICFILES_DIRS[0]+"/etc/hosts", "r+") as myfile:
        #     if not '\n{}\tsurvey.com'.format(settings.MAIN_IP) in myfile.read():
        #         myfile.write('\n{}\tsurvey.com'.format(settings.MAIN_IP)) 
        #     if not "\n{}\t{}".format(settings.MAIN_IP,self.domain) in myfile.read():            
        #         myfile.write("\n{}\t{}".format(settings.MAIN_IP,self.domain))
        # command = 'sudo cp {}/etc/hosts /etc/hosts'.format(settings.STATICFILES_DIRS[0])
